...
const fetchPhotos = async () => {
  ...
  if (cached) {
    ...
    photos.value = cachedData;
    defaultFeed.value = cachedData;
    loading.value = false;
    return;
  }
...
const handleSearchResults = (results) => {
  photos.value = results.length > 0 ? results : defaultFeed.value;
};
  ...
  <div class="min-h-screen bg-gray-950 text-white p-8">
    ...
    >
      <div
        ...
        class="bg-gray-900 rounded-lg overflow-hidden shadow-lg flex flex-col h-[430px] w-full cursor-pointer transition hover:transform relative group border border-[#f09433] hover:border-[#dc2743]"
      >
        <!-- Botão de favorito -->
        <button
          @click="toggleFavorite(photo, $event)"
          class="absolute top-3 right-3 z-10 p-2 rounded-full bg-black/50 backdrop-blur-sm hover:bg-black/70 transition-all duration-200 opacity-0 group-hover:opacity-100"
          :class="{
            '!opacity-100': isFavorite(photo.id),
          }"
        ...
        >
          <svg
            ...
            :class="{
              ...
              'text-white hover:text-red-400': !isFavorite(photo.id),
            }"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
    ...
    <PhotoModal
      ...
      @close="closeModal"
      @toggle-favorite="toggleFavorite(selectedPhoto, $event)"
    />
  </div>
</template>
